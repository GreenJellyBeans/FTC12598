...
public class ITask_miraidk implements TaskInterface {
    final String THIS_COMPONENT = "TASK_SD"; // For "Task simple drive"
    final RuntimeSupportInterface rt;
    final SubSysmiraidk drive;
    final LoggingInterface log;

    public ITask_miraidk(RuntimeSupportInterface rt, SubSysmiraidk ssDrive) {
        this.rt = rt;
        this.drive = ssDrive;
    ...
    public void loop() {
        ...
        if (fwd_bkwd<0.25 && fwd_bkwd>-0.25 && rt_lt<0.25 && rt_lt>-0.25 && !rt.gamepad2().right_bumper() && !rt.gamepad2().left_bumper()) {
            drive.leftDrive.setPower(0);
            drive.fleftDrive.setPower(0);
            drive.frightDrive.setPower(0);
            drive.rightDrive.setPower(0);
        ...
        if (rt.gamepad2().right_bumper() && fwd_bkwd<0.25 && fwd_bkwd>-0.25 && rt_lt<0.25 && rt_lt>-0.25) {         //right bumper makes the robot spin clockwise
            drive.leftDrive.setPower(0.5);
            drive.fleftDrive.setPower(0.5);
            drive.frightDrive.setPower(-0.5);
            drive.rightDrive.setPower(-0.5);
        }else if (rt.gamepad2().left_bumper() && fwd_bkwd<0.25 && fwd_bkwd>-0.25 && rt_lt<0.25 && rt_lt>-0.25) {    //left bumper makes the robot spin counterclockwise
            drive.leftDrive.setPower(-0.5);
            drive.fleftDrive.setPower(-0.5);
            drive.frightDrive.setPower(0.5);
            drive.rightDrive.setPower(0.5);
